...
fn first_hop<'a>(vec_entities: &'a Vec<String>, vec_identifiers: &'a Vec<String>, capacity: usize) -> FxHashMap<&'a str, Vec<&'a str>> {
    let mut entity_to_identifier: FxHashMap<&str, Vec<&str>> = FxHashMap::with_capacity_and_hasher(capacity,  Default::default());
    let mut identifier_to_entity: FxHashMap<&str, Vec<&str>> = FxHashMap::default();
    let mut entity_to_entity: FxHashMap<&str, FxHashSet<&str>> = FxHashMap::with_capacity_and_hasher(capacity,  Default::default()); // Use HashSet for deduplication
    ...
    for i in 0..vec_entities.len() {
        ...
        if let Some(entity_set) = entity_to_entity.get_mut(entity_key.as_str()) {
            ...
            if let Some(entity_vec) = identifier_to_entity.get(identifier.as_str()) {
                let entity_set: FxHashSet<&str> = entity_vec.iter().cloned().collect();
    ...
    let entity_to_entity: FxHashMap<&str, Vec<&str>> = entity_to_entity
        .into_iter()
        .filter(|(k, v)| {
            !v.is_empty() &&
            v.len() > 1 ||
...
fn multihop_iter<'a>(
    mut entity_to_entity: FxHashMap<&'a str, Vec<&'a str>>,
    mut shared_entities_length: FxHashMap<&'a str, usize>,
    capacity: usize
) -> (FxHashMap<&'a str, Vec<&'a str>>, FxHashMap<&'a str, usize>, bool) {
    let mut entity_to_entity_enhanced: FxHashMap<&str, FxHashSet<&str>> = FxHashMap::with_capacity_and_hasher(capacity, Default::default());
    let mut any_chain_got_longer: bool = false;

    for (entity, shared_entities) in entity_to_entity.clone().into_iter() {
        let mut all_entities: FxHashSet<&str> = FxHashSet::default();
    ...
    let entity_to_entity_enhanced: FxHashMap<&str, Vec<&str>> = entity_to_entity_enhanced
        .into_iter()
        .map(|(k, v)| (k, v.into_iter().collect()))
        .collect();
...
fn main() {
    ...
    let mut entity_to_entity: FxHashMap<&str, Vec<&str>> = first_hop(&vec_entities, &vec_identifiers, capacity);

    // executing the first hop
    let mut any_chain_got_longer: bool = true;
    let mut shared_entities_length: FxHashMap<&str, usize> = FxHashMap::with_capacity_and_hasher(capacity, Default::default());
